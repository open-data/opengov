...
class WebformElementTermsOfServiceTest extends WebformElementBrowserTestBase {
  ...
  public function testTermsOfService() {
    ...
    if (version_compare(\Drupal::VERSION, '11', '<')) {
      ...
      $assert_session->responseContains('<input data-drupal-selector="edit-terms-of-service-default" type="checkbox" id="edit-terms-of-service-default" name="terms_of_service_default" value class="form-checkbox required" required="required" />');
    }
    ...
    // Check link.
    $assert_session->responseContains('<label for="edit-terms-of-service-link" class="option">I agree to the <a href="/node/1">terms of service</a>. (link)</label>');
