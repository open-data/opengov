...
class ActionConfigurationTest extends WebDriverTestBase {
  ...
  public function testActionCreation() {
    ...
    $id = 'change_moderation_state_of_content_new';
    $label = 'Change moderation state of content to draft';
    $page->fillField('label', $label);
    $assert_session->waitForText('change_moderation_state_of_content_to_draft');
    $page->pressButton('Edit');
    $assert_session->waitForText('Machine-readable name');
    $page->fillField('id', $id);
    $assert_session->waitForText($id);
    // Trigger the AJAX.
