...
((Drupal, settings, once) => {
  const colorSchemeOptions = settings.bootstrap.colorSchemes;
  ...
  function announceFieldChange(changedInput) {
    ...
    const announcement = Drupal.t(
      '@fieldName has changed to @fieldValue',
      {
  ...
  function setColorScheme({ target }) {
    ...
    const selectedColorScheme =
      colorSchemeOptions[target.value].colors;

    if (selectedColorScheme) {
      Object.entries(selectedColorScheme).forEach(([key, color]) => {
        document
          .querySelectorAll(
            `input[name="${key}"], input[name="${key}_visual"]`
          )
          .forEach((input) => {
      ...
      document
        .querySelectorAll(`input[data-bootstrap-custom-color]`)
        .forEach((input) => {
  ...
  /**
   * Initializes bootstrap theme-settings color picker.
  ...
  function initColorPicker(textInput) {
    ...
    colorInput.setAttribute(
      'data-bootstrap-custom-color',
      textInput.getAttribute('data-bootstrap-custom-color')
    );
   ...
   */
  Drupal.behaviors.bootstrapColorPicker = {
    attach: () => {
      const colorSchemeSelect = once(
        'bootstrap-color-picker',
        '[data-drupal-selector="edit-bootstrap-bootstrap-color-scheme"]'
      );
      ...
      const colorTextInputs = once(
        'bootstrap-color-picker',
        '[data-drupal-selector="bootstrap-color-picker"] input[type="text"]'
      );
